...
ax1.plot(lam_data.time.to_value(u.hr), lam_data.lightcurve('total',wl_index,normalize=0), '#544F43', label='Lambertian rad.')
obs = lam_data.lightcurve('total',wl_index)
obs = obs/obs[0] + np.random.normal(scale=lam_data.lightcurve('noise',wl_index)/obs[0])
ax1.scatter(lam_data.time.to_value(u.hr), obs, c='#544F43',marker='.',s=1,alpha=1)
ax1.plot(sol_data.time.to_value(u.hr), sol_data.lightcurve('total',wl_index,normalize=0), '#E28221', label='$u_1=0.5,~u_2=0.15$')
obs = sol_data.lightcurve('total',wl_index)
obs = obs/obs[0] + np.random.normal(scale=sol_data.lightcurve('noise',wl_index)/obs[0])
ax1.scatter(sol_data.time.to_value(u.hr), obs, c='#E28221', marker='.',s=1,alpha=1)
ax1.plot(spt_data.time.to_value(u.hr), spt_data.lightcurve('total',wl_index,normalize=0), '#54B295', label='Spotted')
obs = spt_data.lightcurve('total',wl_index)
obs = obs/obs[0] + np.random.normal(scale=spt_data.lightcurve('noise',wl_index)/obs[0])
ax1.scatter(spt_data.time.to_value(u.hr), obs, c='#54B295', marker='.',s=1,alpha=1)
