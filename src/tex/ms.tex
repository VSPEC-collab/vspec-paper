...
The flow of \vspec{}'s code is broken into three parts:
\begin{enumerate}
    \item Read in configuration \\
    \item Compute planetary spectra \\
    \item Compute stellar spectra
\end{enumerate}

The main object that a user interacts with is the \texttt{ObservationModel}. This object can be initialized by passing it either a YAML file containing
all of the simulation information, or a \texttt{InternalParameters} object encoded with the same information. Upon initialization a local working directory
(by convention \texttt{.vspec}) is created to store simulation outputs and intermediate files.

The second step is initiated by calling the \texttt{ObservationModel.build\_planet()} method. This is where all the interaction with PSG takes place.
First, your 3D Global Circulation Model (GCM) is uploaded to PSG via the API's \texttt{set} method. Then, PSG is updated with information about the observation
that does not change with time, such as instrument parameters and system information. Before entering the main loop \vspec{} computes the geometry of the system
at each epoch, storing it for later. \vspec{} then iterates through each phase, making four calls to the PSG API:
\begin{enumerate}
    \item An \texttt{upd} request setting the geometric parameters for this epoch. \\
    \item An \texttt{all} request to retrieve the combined light spectrum. \\
    \item A second \texttt{upd} request to set the stellar flux to zero. \\
    \item A second \texttt{all} request to retrieve the thermal emission of the planet.
\end{enumerate}

Results from these calls are stored locally in the \texttt{.fits} format.

The last step is to replace the stellar spectra used by PSG with \vspec{}'s stellar model. First, this model is initialized based on the user-passed parameters.
One consideration made during this step is that a user may want a shorter cadence here than is necessary to resolve temporal features in the planetary spectrum,
so the geometry is recalculated and the planetary spectrum at each epoch is a linear interpolation of the results from the previous step. After this setup
\vspec{} enters the main loop, where it computes the composite stellar spectrum based on the stellar model and spectral grid. For each epoch it returns this stellar
spectrum as seen by the observer, the spectrum as seen by the planet, the reflected spectrum, the planetary thermal spectrum, the total combined spectrum, and the
total noise spectrum. The data at each epoch is stored in a \texttt{.fits} file.
...
parse all output files and conveniently access 1D spectra or lightcurves.
